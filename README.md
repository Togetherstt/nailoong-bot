# Nailoong Bot

基于 `NoneBot2 + OneBot V11 + NapCatQQ` 的自定义 QQ 机器人项目。

当前已实现的核心功能：

- 奶龙表情包记忆
  - `@机器人 /添加奶龙 [名称可选]`
  - `@机器人 /随机奶龙`
  - `@机器人 /奶龙列表`
  - `@机器人 /删除奶龙 <序号|名称|最近一个>`
  - `@机器人 /撤销删除奶龙`
- 昔涟口吻改写
  - `@机器人 /开启昔涟模式`
  - `@机器人 /关闭昔涟模式`
  - `引用文本 + @机器人 /昔涟改写`
  - `引用文本 + @机器人 /昔涟回复`
- 笑话梗库
  - `引用消息 + /上传笑话 [tag]`
  - `/随机笑话 [tag]`
  - `/模糊查找笑话 关键字段`
  - `/编号查找 编号`
  - `/删除笑话 编号`
  - `/笑话 help`
  - `@机器人 /help`
- 杀戮尖塔猜卡
  - `@机器人 /猜卡`
  - `@机器人 /提示`
  - `@机器人 /结束猜卡`
  - `@机器人 /猜卡测试 <卡牌名或ID>`
  - `python scripts/test_sts_card_guess_local.py --card 暴走`
  - `/猜卡 help`
- 谐音盒
  - `@机器人 /添加首字母 <2到5位字母> [@群友可选]`
  - `@机器人 /绑定群友 <2到5位字母> @群友`
  - `@机器人 /解绑群友 <2到5位字母>`
  - `@机器人 /首字母列表`
  - `@机器人 /删除首字母 <2到5位字母>`
  - `@机器人 /谐音盒`
  - `引用一段文本 + /盒`

## 1. 适用场景

这个仓库只负责机器人逻辑层，不负责 QQ 客户端本体。

推荐部署结构：

- `NapCatQQ` 负责登录 QQ 小号、接收消息、发送消息
- 本项目负责通过 `OneBot V11 正向 WebSocket` 接收事件并执行业务逻辑

如果你是 fork 以后自己部署，这份文档就是按“从零部署到可用”来写的。

## 2. 项目结构

```text
Nailoong Bot/
├─ AGENTS.MD
├─ LICENSE
├─ README.md
├─ bot.py
├─ pyproject.toml
├─ requirements.txt
├─ .env.example
├─ data/
│  ├─ homophone_box/
│  └─ nailoong/
├─ src/
│  └─ plugins/
│     ├─ basic_reply/
│     ├─ homophone_box/
│     ├─ joke_library/
│     ├─ nailoong_memory/
│     ├─ sts_card_guess/
│     ├─ utils/
│     ├─ vision_api/
│     └─ xilian_style/
└─ tests/
```

## 3. 环境准备

### Python

要求：

- `Python 3.10+`

### 安装依赖

使用你自己的虚拟环境或 Conda 环境均可。

```powershell
pip install -r requirements.txt
```

如果你使用 Conda，也可以：

```powershell
conda create -n nailoong-bot python=3.11 -y
conda activate nailoong-bot
pip install -r requirements.txt
```

## 4. 创建配置文件

把 `.env.example` 复制成 `.env`，然后按你的实际环境修改。

### 推荐最小配置

```env
ENVIRONMENT=dev
LOG_LEVEL=DEBUG
HOST=127.0.0.1
PORT=8080
COMMAND_START=["/"]
ONEBOT_ACCESS_TOKEN=nailoong-local-token
ADMIN_QQ=123456789
HOMOPHONE_GROUP_IDS=123456789,987654321
SUPERUSERS=[]
VISION_API_URL=https://api.example.com/v1/chat/completions
VISION_API_KEY=sk-xxxxxx
XILIAN_API_URL=https://api.example.com/v1/responses
XILIAN_API_KEY=sk-xxxxxx
XILIAN_API_MODEL=gpt-5.4
XILIAN_API_TIMEOUT=20
STS_CARD_DATA_DIR=data/sts_card_guess/cards
```

### 关键字段说明

- `HOST`
  - 机器人监听地址
  - 一般保持 `127.0.0.1`
- `PORT`
  - 机器人监听端口
  - 必须和 NapCatQQ 里配置的正向 WebSocket 端口一致
- `ONEBOT_ACCESS_TOKEN`
  - NapCatQQ 和本项目之间的访问令牌
  - 两边必须完全一致
- `ADMIN_QQ`
  - 管理员 QQ 号
  - 只有这个 QQ 号可以使用奶龙库存管理命令
  - fork 本项目后，你只需要改这个值，就能换成你自己的管理员账号
- `HOMOPHONE_GROUP_IDS`
  - 允许使用谐音盒的群号列表
  - 多个群号用英文逗号分隔
  - 只有这些群里才能触发 `/谐音盒` 和 `/盒`
- `SUPERUSERS`
  - NoneBot 级别的超级用户配置
  - 当前项目并不依赖它做奶龙管理权限判断，但你可以为后续扩展保留
- `VISION_API_URL`
  - 预留给图像识别 / 多模态接口
- `VISION_API_KEY`
  - 对应的 API Key
- `XILIAN_API_URL`
  - 昔涟改写功能的 Responses API 接口地址
  - 留空时会回退到 `VISION_API_URL`
- `XILIAN_API_KEY`
  - 昔涟改写功能的 API Key
  - 留空时会回退到 `VISION_API_KEY`
- `XILIAN_API_MODEL`
  - 昔涟改写功能调用的模型名
- `XILIAN_API_TIMEOUT`
  - 昔涟改写接口超时时间，单位秒
- `STS_CARD_DATA_DIR`
  - 杀戮尖塔猜卡读取卡牌 JSON 的目录
  - 当前仓库默认已内置一份卡池在 `data/sts_card_guess/cards`
  - 你也可以自行改成别的本地目录
  - 如果目录不存在，`/猜卡` 和 `/猜卡测试` 会直接提示无法读取卡池

## 5. 配置 NapCatQQ

### 第一步：登录机器人小号

在 NapCatQQ 中登录你要作为机器人的小号。

要求：

- 这个号能正常收发消息
- 能在群里被 `@`
- 你愿意长期用它作为机器人账号

### 第二步：开启 OneBot V11 正向 WebSocket

在 NapCatQQ 配置里：

1. 选择 `OneBot V11`
2. 连接方式选择 `正向 WebSocket`
3. 地址配置为：

```text
127.0.0.1:8080
```

如果你在 `.env` 中改了 `PORT`，这里也要同步改。

4. Access Token 填：

```text
nailoong-local-token
```

如果你在 `.env` 中改了 `ONEBOT_ACCESS_TOKEN`，这里也要同步改。

## 6. 启动机器人

### 直接运行

```powershell
python bot.py
```

### 或使用 NoneBot 热重载

```powershell
nb run --reload
```

如果启动正常，你会在终端看到 NoneBot 初始化和插件加载日志。

## 7. 功能使用

下面示例里的 `@机器人`，指的是你部署后的 QQ 小号。

### 7.1 奶龙表情包记忆

#### 添加奶龙

先引用一张奶龙图片，或者引用一个 QQ 表情包，然后发送：

```text
@机器人 /添加奶龙 憨笑奶龙
```

名称可省略：

```text
@机器人 /添加奶龙
```

机器人会记录：

- 图片或表情包内容
- 名称
- 添加者 QQ
- 添加时间

#### 随机奶龙

```text
@机器人 /随机奶龙
```

机器人会返回：

- 一张随机奶龙表情包
- 奶龙名称
- 添加者 QQ
- 添加日期

### 7.2 奶龙库存管理

只有 `.env` 里的 `ADMIN_QQ` 可以使用这些命令。

#### 查看库存

```text
@机器人 /奶龙列表
```

#### 按序号删除

```text
@机器人 /删除奶龙 2
```

#### 按名称删除

```text
@机器人 /删除奶龙 憨笑奶龙
```

#### 删除最近一条

```text
@机器人 /删除奶龙 最近一个
```

#### 撤销最近一次删除

```text
@机器人 /撤销删除奶龙
```

### 7.3 谐音盒

#### 添加首字母模式

```text
@机器人 /添加首字母 yzh
```

规则：

- 支持 `2` 到 `5` 位英文字母
- 自动转为小写
- 持久化保存，重启后不会丢失
- 可以在添加时直接绑定群友：

```text
@机器人 /添加首字母 yzh @某个群友
```

#### 查看首字母库

```text
@机器人 /首字母列表
```

#### 绑定群友

如果首字母已经存在，可以补绑或覆盖绑定关系：

```text
@机器人 /绑定群友 yzh @某个群友
```

#### 解绑群友

如果你不想再让某个首字母关联到某位群友：

```text
@机器人 /解绑群友 yzh
```

#### 删除首字母模式

```text
@机器人 /删除首字母 yzh
```

#### 触发谐音盒

先引用一段短文本，再发送：

```text
@机器人 /谐音盒
```

或者更快一点，直接：

```text
/盒
```

这个快捷入口不需要 `@机器人`。

注意：

- 谐音盒只在 `.env` 的 `HOMOPHONE_GROUP_IDS` 指定群内生效
- 私聊和未列入白名单的群不会触发谐音盒

### 7.4 昔涟口吻改写

这个功能依赖文本生成 API。

#### 开启和关闭

只有 `.env` 里的 `ADMIN_QQ` 可以执行：

```text
@机器人 /开启昔涟模式
@机器人 /关闭昔涟模式
```

如果不是管理员，机器人会回复：

```text
您没有管理权限，无法开启
```

#### 昔涟改写

先引用一段不超过 200 字的话，再发送：

```text
@机器人 /昔涟改写
```

机器人会返回一段不超过 200 字的昔涟口吻改写语句。

#### 昔涟回复

先引用一段不超过 200 字的话，再发送：

```text
@机器人 /昔涟回复
```

机器人会返回一段不超过 200 字的昔涟口吻回复语句。

#### 输出约束

- 风格偏少女感、柔和、抒情、朦胧
- 可以适度使用 `U+266A`，也就是 `♪`
- 不能替换成 `🎵`、`♫`、`♬`、`♩`
- 全文最后必须以单个 `♪` 结尾
- 以 `♪` 结尾的句子后面不能再加标点

#### 调用限制

- 昔涟模式未开启时，`/昔涟改写` 和 `/昔涟回复` 不会生效
- 昔涟接口当前不再额外做 1 分钟次数限流
- 如果接口配置错误、模型不可用或上游返回异常，会回复：

```text
昔涟改写服务暂时不可用，请稍后再试。
```

### 7.5 笑话梗库

这一组命令全部都不需要 `@机器人`。

#### 上传笑话

先引用一条聊天消息，再发送：

```text
/上传笑话
```

这样会把这条消息上传到通用笑话库。

如果你想归到某个 tag 下，就发送：

```text
/上传笑话 校园
```

说明：
- 被引用内容可以是普通文字、图片、QQ 表情，或者它们的混合消息。
- 每条笑话都会分配一个全局唯一编号。
- 系统会自动记录归属标签、提供人 QQ、提供时间。
- 上传时会自动去重；如果内容完全重复，不会重复入库，而是直接返回已有笑话的编号和信息。

### 奶龙去重说明

- 奶龙上传已启用双层去重：
  - 第一层：图片原始字节的 `sha256`
  - 第二层：图片感知哈希 `dHash`
- 这意味着：
  - 完全相同的图片文件会被直接判重
  - 不同 QQ 表情但图片视觉内容相同、尺寸相同的情况，也有较大概率被判重
- 当前不依赖 OCR。

#### 随机笑话

从全部笑话里随机抽一条：

```text
/随机笑话
```

从某个 tag 里随机抽一条：

```text
/随机笑话 校园
```

返回内容会附带：
- 笑话编号

说明：
- 随机笑话为了避免显示过于臃肿，只保留笑话内容和编号，不再显示 tag、提供人和时间。

#### 模糊查找笑话

```text
/模糊查找笑话 晚风
```

规则：
- 关键字段不能为空。
- 关键字段不能超过 `10` 个字。
- 机器人会返回当前匹配度最高的一条笑话。

#### 编号查找

```text
/编号查找 12
```

这会直接返回编号为 `12` 的那条笑话。

#### 删除笑话

```text
/删除笑话 12
```

说明：
- 只有 `.env` 里的 `ADMIN_QQ` 可以删除。
- 普通用户调用时会明确提示没有权限。

#### 查看帮助

```text
/笑话 help
```

机器人会返回笑话功能的完整用法说明。

#### 总帮助

```text
@机器人 /help
```

说明：
- 总 `/help` 需要 `@机器人`，避免和其他 bot 的通用 help 冲突。
- 总帮助只会告诉你各个分支插件对应的 help 指令，其中也包含 `/猜卡 help`。

### 7.6 杀戮尖塔猜卡

#### 准备卡池目录

先确保 `.env` 中的 `STS_CARD_DATA_DIR` 指向卡牌 JSON 目录。当前仓库已经内置了一份卡池，推荐直接使用：

```env
STS_CARD_DATA_DIR=data/sts_card_guess/cards
```

当前实现会递归读取该目录下的 `.json` 卡牌文件，并使用以下来源映射：

- `defect` -> `故障机器人`
- `silent` -> `静默猎手`
- `ironclad` / `铁甲战士` -> `铁血战士`
- `regent` -> `储君`
- `necrobinder` -> `亡灵契约师`

#### 开始游戏

在群里发送：

```text
@机器人 /猜卡
```

规则说明：

- 同一个群同一时间只能有一局猜卡
- 开局会在“卡名字数、类型、来源、费用”这 4 条基础信息里随机给出 3 条
- 由于卡名本体必须隐藏，所以这里展示的是“卡名字数”，不是直接公布名称
- 卡牌描述中的非数字、非标点字符会先被隐藏成 `_`，并且下划线之间会加空格，避免 QQ 把它们连成一条
- 10 秒后若还没人猜中，会补上第 4 条基础信息
- 之后每过 60 秒，会按“开局时隐藏字总数”的 25% 固定揭示一批文字
- 描述提示一共只会揭示 4 批；第 4 批揭示完成后若仍无人猜中，则本局失败并自动结束

#### 猜测格式

```text
@机器人 暴走
```

要求：

- 必须输入完整卡牌名
- 猜对后会立即宣布成功并结束本局
- 猜错后只提示“猜错了，游戏继续”

#### 手动推进提示

```text
@机器人 /提示
```

说明：

- 会立刻跳到下一阶段提示
- 跳过后计时器重置为 60 秒

#### 手动结束

```text
@机器人 /结束猜卡
```

这会直接结束当前群的这一局，并公布答案。

#### 群内测试接口

为了方便你验收游戏流程，额外提供了管理员测试入口：

```text
@机器人 /猜卡测试 暴走
```

或者：

```text
@机器人 /猜卡测试 rampage
```

说明：

- 只有 `.env` 中 `ADMIN_QQ` 对应的管理员可以使用这个接口
- 这会用你指定的卡牌直接开一局，方便测试定时、提示和答案判定

#### 本地测试接口

如果你不想经过 NapCatQQ，也不想真的往群里发消息，可以直接在本地跑核心逻辑测试脚本：

```powershell
python scripts/test_sts_card_guess_local.py --card 暴走
```

也可以用 card id：

```powershell
python scripts/test_sts_card_guess_local.py --card rampage
```

说明：

- 这个脚本不依赖 QQ，不依赖 OneBot，不需要启动 NapCatQQ
- 它只测试猜卡核心逻辑：开局提示、手动推进、答案判定、描述遮罩
- 进入脚本后可用命令为：
  - `/提示`
  - `/答案`：直接公布答案，并按失败结束本地测试
  - `/退出`
- 如果你要测试真实的群聊时序、@ 解析和同群单局限制，仍然要再走一次机器人联调

#### 查看帮助

```text
/猜卡 help
```

### 7.7 谐音盒规则

- 引用文本中的有效中文字符数不能超过 `40`
- 候选单元可以是：
  - 单个中文字符
  - 连续英文单词
- 中文按拼音首字母匹配
- 英文按单词首字母匹配
- 对于以空格分隔的英文单词，还支持“单词整体命中”
- 英文整词的边界只认“空格、中文字符、文本边界”
  - 例如首字母库里存在 `hello`
  - 引用文本里出现独立单词 `hello`
  - `杨hello魂` 这种中英紧贴写法也会把 `hello` 识别为一个完整英文单词
  - `hello-world` 这种写法不会被拆成两个英文整词
  - 也会直接命中
- 选取的候选单元个数，和首字母模式长度一致
  - `yz` 匹配 2 个候选单元
  - `yzh` 匹配 3 个候选单元
  - `yzhh` 匹配 4 个候选单元
  - `yzhhl` 匹配 5 个候选单元
- 不再强依赖中文字符；如果引用文本是纯英文，只要存在可识别的英文单词，也可以参与匹配
- 纯英文引用既可以走“英文整词直接命中”，也可以走“按英文单词首字母组合匹配”
- 如果某个首字母绑定了群友，命中时会在结果后显示群昵称
- 命中结果超过 `10` 条时，只随机显示 `10` 条
- 同一条引用文本短时间连续触发时，会尽量轮换结果
- 同一条被引用消息有 `5` 秒冷却
  - 冷却期内再次触发，只回复：

```text
已经盒过了。
```

- 瞬时请求太多时会启用内部缓冲
  - 队列满了会回复：

```text
当前盒子太忙，请稍后再试。
```

### 7.8 绑定群友后的输出格式

如果某个首字母没有绑定群友：

```text
盒出了：
<杨州话>
```

如果某个首字母绑定了群友：

```text
盒出了：
<杨州话>（张三）
```

说明：

- 这里显示的是群昵称 / 群名片
- 不会再次 `@` 该群友，避免骚扰
- 如果后续执行 `/解绑群友 yzh`，则会恢复为不带括号昵称的原始输出

纯英文整体命中的例子：

如果姓名库里有：

```text
hello
```

引用文本是：

```text
hello world
```

则也可能输出：

```text
盒出了：
<hello>
```

## 8. 数据存储

本项目当前不依赖数据库，数据保存在本地：

- 奶龙索引：`data/nailoong/index.json`
- 奶龙回收区：`data/nailoong/deleted.json`
- 奶龙图片：`data/nailoong/images/`
- 奶龙回收站图片：`data/nailoong/trash/`
- 谐音盒首字母库：`data/homophone_box/initials.json`
- 昔涟模式开关：`data/xilian_style/mode_state.json`
- 笑话库索引：`data/joke_library/index.json`
- 笑话库图片：`data/joke_library/images/`

这意味着：

- 重启 bot 后，奶龙记忆、首字母库和昔涟模式开关都不会丢失
- 重启 bot 后，笑话库也不会丢失
- 猜卡游戏本身不持久化，正在进行中的猜卡局在 bot 重启后会直接结束
- 如果你要迁移机器人，记得连同 `data/` 一起迁移

## 9. 最小验证流程

部署完成后，至少验证这些步骤：

1. 启动 `bot.py`
2. 确认 NapCatQQ 已成功连接
3. 引用一张奶龙图片并发送：

```text
@机器人 /添加奶龙 憨笑奶龙
```

4. 发送：

```text
@机器人 /随机奶龙
```

5. 使用你的管理员 QQ 测试：

```text
@机器人 /奶龙列表
```

6. 测试删除：

```text
@机器人 /删除奶龙 1
```

7. 测试撤销：

```text
@机器人 /撤销删除奶龙
```

8. 添加首字母：

```text
@机器人 /添加首字母 yzh
```

9. 如果需要，测试绑定群友：

```text
@机器人 /绑定群友 yzh @某个群友
```

10. 如果需要，测试解绑：

```text
@机器人 /解绑群友 yzh
```

11. 用管理员 QQ 测试猜卡指定卡牌开局：

```text
@机器人 /猜卡测试 暴走
```

12. 再测试正常随机开局：

```text
@机器人 /猜卡
```

13. 用：

```text
@机器人 /提示
```

确认阶段推进正常，再用完整卡名猜测，或用：

```text
@机器人 /结束猜卡
```

确认手动结束正常。

11. 在白名单群内引用一段短文本并发送：

```text
@机器人 /谐音盒
```

12. 再次对同一条引用消息在 5 秒内重复触发，确认是否返回：

```text
已经盒过了。
```

13. 由管理员发送：

```text
@机器人 /开启昔涟模式
```

14. 引用一句不超过 200 字的话并测试：

```text
@机器人 /昔涟改写
```

15. 再测试：

```text
@机器人 /昔涟回复
```

## 10. 单元测试

```powershell
python -m unittest discover -s tests -v
```

## 11. 常见问题

### 机器人没有回复

优先检查：

- NapCatQQ 是否已连接到 `HOST:PORT`
- `ONEBOT_ACCESS_TOKEN` 是否两边一致
- 小号是否真的被 `@`
- 消息格式是否符合命令要求

### 奶龙管理命令没反应

检查：

- `.env` 中是否正确设置了 `ADMIN_QQ`
- 你当前发命令的账号是否就是这个 QQ

### 谐音盒完全没反应

优先检查：

- 当前群号是否已经写入 `.env` 的 `HOMOPHONE_GROUP_IDS`
- 修改 `.env` 后是否已经重启 bot
- 当前是不是私聊，或不是白名单群

### 谐音盒提示“已经盒过了”

这是正常行为，表示同一条被引用消息还在 5 秒冷却内。

### 谐音盒提示“当前盒子太忙，请稍后再试。”

这是正常保护机制，说明短时间内请求太多。稍后重试即可。

## 12. License

本项目使用 [MIT License](./LICENSE)。
